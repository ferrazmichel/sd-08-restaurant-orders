...
def analyze_log(path_to_file):
    with open("data/mkt_campaign.txt", "w") as campaign:
        first = prato_mais_pedido_por_maria(path_to_file)
        second = quantas_vezes_arnaldo_pediu(path_to_file)
        third = joao_nunca_pediu(path_to_file)
        fourth = joao_nunca_foi(path_to_file)
...
def prato_mais_pedido_por_maria(path_to_file):
...
def quantas_vezes_arnaldo_pediu(path_to_file):
    with open(path_to_file) as orders_file:
        orders = csv.reader(orders_file, delimiter=",")
        count = 0
        for row in orders:
...
def joao_nunca_pediu(path_to_file):
    with open(path_to_file) as orders_file:
        orders = csv.reader(orders_file, delimiter=",")
...
def joao_nunca_foi(path_to_file):
    with open(path_to_file) as orders_file:
        orders = csv.reader(orders_file, delimiter=",")
